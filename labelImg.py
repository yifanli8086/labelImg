...
class MainWindow(QMainWindow, WindowMixin):
    ...
    def __init__(self, defaultFilename=None, defaultPrefdefClassFile=None):
        ...
        self.diffcButton = QCheckBox(u'Jaywalking')
        ...
        # Add chris
        Shape.jaywalking = self.jaywalking
    ...
    def btnstate(self, item= None):
        """ Function to handle jaywalking examples
        ...
        jaywalking = self.diffcButton.isChecked()
        ...
        # Checked and Update
        try:
            if jaywalking != shape.jaywalking:
                shape.jaywalking = jaywalking
                self.setDirty()
    ...
    def loadLabels(self, shapes):
        s = []
        for label, points, line_color, fill_color, jaywalking in shapes:
    ...
    def labelSelectionChanged(self):
        ...
        if item and self.canvas.editing():
            ...
            shape = self.itemsToShapes[item]
            # Add Chris
            self.diffcButton.setChecked(shape.jaywalking)
    ...
    def newShape(self):
        ...
        if not self.useDefaultLabelCheckbox.isChecked() or not self.defaultLabelTextLine.text():
            ...
            if self.singleClassMode.isChecked() and self.lastLabel:
                ...
                nextLabel = int(self.prevLabelText) + 1
                text = self.labelDialog.popUp(text=str(nextLabel))
                self.lastLabel = text
        else:
        ...
        if text is not None:
            ...
            self.setDirty()
            if nextLabel not in self.labelHist:
                self.labelHist.append(nextLabel)  
    ...
    def openPrevImg(self, _value=False):
        ...
        if self.filePath is not None:
            print('Prev')
            # This would automatically overwrite next frame's xml with current xml
            # self.saveFile()
        if not self.mayContinue():
            return
    ...
    def saveFile(self, _value=False):
        if self.defaultSaveDir is not None and len(ustr(self.defaultSaveDir)):
            if self.filePath:
                ...
                nextFrame = int(os.path.splitext(imgFileName)[0]) + 1
                
                # Combine index and extention
            ...
            imgFileName = os.path.basename(self.filePath)
            nextFrame = int(os.path.splitext(imgFileName)[0]) + 1
            nextName = imgFileName
            if nextFrame > 9999:
                nextName = str(nextFrame)
                print(nextName)
            elif nextFrame > 999:
                nextName = str('0'+str(nextFrame))
                print(nextName)
            elif nextFrame > 99:
                nextName = str('00'+str(nextFrame))
                print(nextName)
            elif nextFrame > 9:
                nextName = str('000'+str(nextFrame))
                print(nextName)
            else:
                nextName = str('0000'+str(nextFrame))
                print(nextName)
            saveNextFile = nextName + XML_EXT
            savedNextPath = os.path.join(imgFileDir, saveNextFile)
            # If next frame's label doesn't exist, save Same Label to Next Frame 
            if os.path.exists(savedNextPath) is False:
                self._saveFile(savedNextPath)
            # Combine index and extention
            savedFileName = os.path.splitext(imgFileName)[0] + XML_EXT
    ...
    def loadPredefinedClasses(self, predefClassesFile):
        if os.path.exists(predefClassesFile) is True:
            with codecs.open(predefClassesFile, 'r', 'utf8') as f:
                print('Loading pre-defined class is disabled, check Ln 1327') 
                #self.labelHist = [str(pedestrianCounter)]
                #for line in f:
                #    line = line.strip()
